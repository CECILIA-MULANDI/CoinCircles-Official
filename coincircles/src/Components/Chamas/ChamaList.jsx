...
{!isMember(chama, userAddress) && (
    <>
        {chama.visibility === 0 ? (
            <button style={styles.button} onClick={() => handleJoinChama(chama.name)}>
                Join Chama
            </button>
        ) : (
...
{isMember(chama, userAddress) && !chama.hasContributionStarted && !isContributionStarted && (
    <button
        style={styles.button}
        onClick={() => handleContributeFunds(chama.name)}
        disabled={hasContributedInCurrentRound(chama.id, userAddress)}
    >
        {hasContributedInCurrentRound(chama.id, userAddress) ? 'Contributed' : 'Contribute Funds'}
    </button>
)}
<button style={styles.button} onClick={() => handleSelectChama(chama.name)}>
    Select Chama
</button>
</div>
))}
