...
contract CoinCircles {
   ...
   function contributeFunds(string memory _name) public payable {
    ...
    // No need to enforce a strict equality check, allow some flexibility
    ...
    function startVoting(string memory _name) public {
        uint256 chamaId = getChamaId(_name);
        require(chamaExists(_name), "Chama does not exist");
        require(chamas[chamaId].owner == msg.sender, "Only the owner can start the voting process");
        
        // Ensure that contributions have been made by all members in the current round
        require(chamas[chamaId].hasContributionStarted && contributorsInCurrentRound[chamaId].length == chamas[chamaId].listOfMembers.length, "All members have not contributed in the current round");
    ...
    function voteForRecipient(string memory _name, address recipient) public {
        uint256 chamaId = getChamaId(_name);
        require(chamaExists(_name), "Chama does not exist");
        require(isMember(_name, msg.sender), "User is not a member of this chama");
        Voting storage voting = votings[chamaId];
        ...
        // Set the recipient address to the provided parameter
        voting.recipient = recipient;

        // Record the vote after setting the recipient
        voting.votes[msg.sender] = true;
        voting.totalVotes++;

        emit VoteCast(_name, msg.sender, recipient);
  ...
  function distributeFunds(string memory _name) internal {
    ...
    require(chamas[chamaId].currentRound <= chamas[chamaId].numberOfRounds, "Chama has ended");
    ...
    emit FundsReceived(_name, recipient, amountToDistribute);
}
    ...
    function hasContributedInCurrentRound(uint256 chamaId, address member) public view returns (bool) {
        return hasContributed[member][chamas[chamaId].currentRound];
    }
    ...
    function getCurrentRecipient(string memory _name) internal view returns (address) {
        uint256 chamaId = getChamaId(_name);
        return chamas[chamaId].listOfMembers[chamas[chamaId].currentRound - 1];
    }
